(function(){
  ...
  const partnersCarousel=isV14?document.querySelector('.club-partners-carousel'):null;
  const partnersBand=isV14?document.querySelector('.club-partners-band'):null;
  ...
  function card(item){
    ...
    if(isV14){
      ...
      if(item.phone){
        const label=item.phone_label?`${item.phone_label} ${item.phone}`:`Tel. ${item.phone}`;
        const phone=make('a','collaborator-phone',label);
  ...
  function addCollaboratorsToPartnerCarousel(items){
    ...
    if(partnersBand&&!partnersBand.querySelector('.supporters-label')){
      partnersBand.appendChild(make('div','supporters-label','Nos apoyan'));
      partnersBand.setAttribute('aria-label','Patrocinadores y colaboradores del Club Casas de Haro BTT');
    }
    ...
    items.forEach(item=>{
      ...
      a.className='club-partner-card is-collaborator';
      a.href='patrocinadores.html#colaboradores-club';
      a.setAttribute('aria-label',`${item.name||'Colaborador'} · colaborador del club`);

      const img=document.createElement('img');
      if(item.logo){
        img.src=String(item.logo).replace(/^\/+/, '');
        img.alt=item.name||'Colaborador del club';
      }else{
        img.src='../assets/logo-cdh-oficial-2026.png';
        img.alt=item.name||'Colaborador del club';
      }
      img.loading='lazy';
      img.decoding='async';
      ...
      const copy=make('div','club-partner-copy');
      copy.appendChild(make('span','','COLABORADOR DEL CLUB'));
      copy.appendChild(make('h3','',item.name||'Colaborador'));
      copy.appendChild(make('p','',item.carousel_text||'Colaborador del Club Casas de Haro BTT.'));
      a.appendChild(copy);
      partnersCarousel.appendChild(a);
    });

    const cards=[...partnersCarousel.querySelectorAll('.club-partner-card')];
    const slotSeconds=4;
    const totalSeconds=Math.max(slotSeconds,cards.length*slotSeconds);
    cards.forEach((el,index)=>{
      el.style.animationDuration=`${totalSeconds}s`;
      el.style.animationDelay=`${index*slotSeconds}s`;
    });
  }
  ...
  fetch('data/collaborators.json',{cache:'no-store'})
    ...
    .catch(()=>{
      ...
      addCollaboratorsToPartnerCarousel([]);
    });
})();
